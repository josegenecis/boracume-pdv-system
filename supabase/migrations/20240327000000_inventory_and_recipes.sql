...
-- Políticas (Policies)
CREATE POLICY Permitir leitura/escrita de insumos ao dono ON public.ingredients FOR ALL USING (auth.uid() = restaurant_id);
CREATE POLICY Permitir leitura/escrita de receitas ao dono ON public.product_recipes FOR ALL USING (
    EXISTS (SELECT 1 FROM public.products p WHERE p.id = product_id AND p.restaurant_id = auth.uid())
);
CREATE POLICY Permitir leitura/escrita de movimentacoes ao dono ON public.stock_movements FOR ALL USING (auth.uid() = restaurant_id);
