...
export async function loadTeamEmployees(restaurantId: string, includeSensitive = false): Promise<TeamEmployee[]> {
  const [employeesResult, compensationResult, rolesResult, permissionsResult, appsResult] = await Promise.all([
    db.from('employees').select('*').eq('restaurant_id', restaurantId).order('full_name'),
    includeSensitive
      ? db.from('employee_compensation').select('*').eq('restaurant_id', restaurantId)
      : Promise.resolve({ data: [], error: null }),
    db.from('employee_roles').select('employee_id, role_code').eq('restaurant_id', restaurantId),
    db.from('employee_permissions').select('employee_id, permission_code, allowed').eq('restaurant_id', restaurantId),
    db.from('employee_app_access').select('employee_id, app_code, enabled, configuration').eq('restaurant_id', restaurantId),
  ]);
...
export async function saveTeamEmployee(restaurantId: string, form: EmployeeFormValue) {
  ...
  const { data, error } = await db.rpc('save_team_employee', {
    ...
    p_app_configuration: {
      driver: { vehicle_type: form.driver_vehicle_type, vehicle_plate: form.driver_vehicle_plate },
    },
  });
  if (error) throw error;
  return String(data);
...
export async function loadSchedules(restaurantId: string): Promise<WorkSchedule[]> {
  ...
  if (error) throw error;
  return rows<any>({ data }).map((item) => ({ ...item, first_start: item.first_start?.slice(0, 5) || null, first_end: item.first_end?.slice(0, 5) || null, second_start: item.second_start?.slice(0, 5) || null, second_end: item.second_end?.slice(0, 5) || null }));
}

export async function saveSchedule(restaurantId: string, schedule: WorkSchedule) {
  const payload = { ...schedule, restaurant_id: restaurantId, updated_at: new Date().toISOString() };
  const result = schedule.id
    ? await db.from('employee_work_schedules').update(payload).eq('id', schedule.id).eq('restaurant_id', restaurantId)
    : await db.from('employee_work_schedules').upsert(payload, { onConflict: 'employee_id,weekday,effective_from' });
  const { error } = result;
  if (error) throw error;
}
...
export async function loadAttendanceData(restaurantId: string, startDate: string, endDate: string) {
  ...
  const [entriesResult, occurrencesResult, settingsResult] = await Promise.all([
    db.from('employee_time_clock_events').select('id,employee_id,waiter_id,event_type,occurred_at,status').eq('user_id', restaurantId).gte('occurred_at', start).lte('occurred_at', end),
    db.from('employee_time_clock_occurrences').select('employee_id,waiter_id,occurrence_type,start_date,end_date,affects_expected_hours,paid,status').eq('user_id', restaurantId).lte('start_date', endDate).gte('end_date', startDate),
    db.from('employee_payroll_settings').select('*').eq('restaurant_id', restaurantId).maybeSingle(),
  ]);
...
export async function loadTeamAlerts(restaurantId: string) {
  const refresh = await db.rpc('refresh_employee_team_alerts', { p_restaurant_id: restaurantId });
...
export async function loadMonthlyFinancialInputs(restaurantId: string, startDate: string, endDate: string) {
  ...
  const [commissionsResult, advancesResult, closingsResult, accountsResult] = await Promise.all([
    db.from('employee_commissions').select('*').eq('restaurant_id', restaurantId).gte('competence_date', startDate).lte('competence_date', endDate).neq('status', 'cancelled'),
    db.from('employee_advances').select('*').eq('restaurant_id', restaurantId).gte('advance_date', startDate).lte('advance_date', endDate).neq('status', 'reversed'),
    db.from('employee_payroll_closings').select('*,employee_payroll_items(*)').eq('restaurant_id', restaurantId).gte('competence_date', startDate).lte('competence_date', endDate).order('created_at', { ascending: false }),
    db.rpc('ensure_default_financial_accounts'),
  ]);
  ...
  const adjustmentsResult = closingIds.length
    ? await db.from('employee_payroll_adjustments').select('*').in('closing_id', closingIds).order('created_at')
    : { data: [], error: null };
...
export async function registerCommission(params: { employeeId: string; amount: number; description: string; competenceDate: string; type?: 'manual' | 'bonus'; operatorId?: string | null }) {
  const { error } = await db.rpc('record_employee_commission', {
    p_employee_id: params.employeeId, p_amount: params.amount, p_competence_date: params.competenceDate,
    p_description: params.description, p_source_type: params.type || 'manual', p_operator_id: params.operatorId || null,
  });
  if (error) throw error;
}
...
export async function loadCommissionConfiguration(restaurantId: string): Promise<{ rules: CommissionRule[]; products: TeamProductOption[] }> {
  const [rulesResult, productsResult] = await Promise.all([
    db.from('employee_commission_rules').select('*').eq('restaurant_id', restaurantId).order('created_at', { ascending: false }),
    db.from('products').select('id,name').eq('user_id', restaurantId).order('name'),
  ]);
  const error = rulesResult.error || productsResult.error;
  if (error) throw error;
  return {
    rules: rows<any>(rulesResult).map((item) => ({ ...item, percentage: item.percentage === null ? null : Number(item.percentage), fixed_amount: item.fixed_amount === null ? null : Number(item.fixed_amount) })),
    products: rows<any>(productsResult),
  };
}

export async function saveCommissionRule(params: {
  restaurantId: string; ruleId?: string | null; employeeId: string; ruleType: CommissionRuleType;
  percentage?: number | null; fixedAmount?: number | null; productId?: string | null; active: boolean;
  startsAt: string; endsAt?: string | null; operatorId?: string | null;
}) {
  const { error } = await db.rpc('save_employee_commission_rule', {
    p_restaurant_id: params.restaurantId, p_rule_id: params.ruleId || null, p_employee_id: params.employeeId,
    p_rule_type: params.ruleType, p_percentage: params.percentage ?? null, p_fixed_amount: params.fixedAmount ?? null,
    p_product_id: params.productId || null, p_active: params.active, p_starts_at: params.startsAt,
    p_ends_at: params.endsAt || null, p_operator_id: params.operatorId || null,
  });
  if (error) throw error;
}

export async function registerPayrollAdjustment(params: {
  closingId: string; employeeId: string; type: 'earning' | 'deduction'; category: string;
  amount: number; description: string; operatorId?: string | null;
}) {
  const { error } = await db.rpc('register_employee_payroll_adjustment', {
    p_closing_id: params.closingId, p_employee_id: params.employeeId, p_adjustment_type: params.type,
    p_category: params.category, p_amount: params.amount, p_description: params.description,
...
export async function registerAdvance(params: { employeeId: string; amount: number; date: string; method: string; accountId: string; note?: string; operatorId?: string | null }) {
  const { error } = await db.rpc('record_employee_advance', {
    p_employee_id: params.employeeId, p_amount: params.amount, p_advance_date: params.date,
    p_payment_method: params.method, p_financial_account_id: params.accountId,
    p_note: params.note || null, p_operator_id: params.operatorId || null,
  });
  if (error) throw error;
}
...
export async function savePayrollPreview(params: { restaurantId: string; competenceDate: string; unitName?: string; items: any[]; snapshot?: Record<string, unknown> }) {
  const { data, error } = await db.rpc('save_employee_payroll_preview', {
    p_restaurant_id: params.restaurantId, p_competence_date: params.competenceDate,
    p_unit_name: params.unitName || null, p_items: params.items, p_snapshot: params.snapshot || {},
  });
  if (error) throw error;
  return String(data);
}
...
export async function setClosingStatus(closingId: string, status: 'approved' | 'review', operatorId?: string | null, reason?: string) {
  const { error } = await db.rpc('set_employee_payroll_closing_status', {
    p_closing_id: closingId, p_status: status, p_operator_id: operatorId || null, p_reason: reason || null,
  });
  if (error) throw error;
}
...
export async function generatePayrollPayables(closingId: string, dueDate: string, operatorId?: string | null) {
  const { data, error } = await db.rpc('generate_employee_payroll_payables', {
    p_closing_id: closingId, p_due_date: dueDate, p_operator_id: operatorId || null,
  });
  if (error) throw error;
  return data;
}
