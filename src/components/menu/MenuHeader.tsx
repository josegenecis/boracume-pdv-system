...
export const MenuHeader: React.FC<MenuHeaderProps> = ({ profile }) => {
  return (
    <div className="relative">
      <div className="h-40 w-full bg-gradient-to-r from-boracume-orange/10 to-boracume-green/5" />
              ...
              <div className="mt-4 flex flex-wrap gap-4 text-sm">
                {profile?.phone && (
                  <div className="flex items-center gap-2 text-gray-700">
                    <Phone className="h-4 w-4 text-boracume-orange" />
                    <span className="font-medium">{profile.phone}</span>
                  </div>
                )}
                {profile?.opening_hours && (
                  <div className="flex items-center gap-2 text-gray-700">
                    <Clock className="h-4 w-4 text-boracume-orange" />
                    <span className="font-medium">{profile.opening_hours}</span>
                  </div>
                )}
                {profile?.address && (
                  <div className="flex items-center gap-2 text-gray-700">
                    <MapPin className="h-4 w-4 text-boracume-orange" />
