...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    // TEMPORARY: Auto-authenticate test user for KDS testing
    const autoAuthenticateTestUser = async () => {
      try {
        console.log('🔄 Auto-authenticating test user for KDS testing...');
        ...
        if (error) {
          ...
          console.log('✅ Auto-auth successful:', data.user.id);
        }
      } catch (error) {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      if (session?.user) {
        ...
        // TEMPORARY: If no session, auto-authenticate test user
        autoAuthenticateTestUser();
      }
